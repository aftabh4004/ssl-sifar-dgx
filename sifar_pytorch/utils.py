...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, lenn=None, header=None):
        i = 0
        if not header:
            header = ''
        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == lenn - 1:
                eta_seconds = iter_time.global_avg * (lenn - i)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                if torch.cuda.is_available():
                    print(log_msg.format(
                        i, lenn, eta=eta_string,
                        meters=str(self),
        ...
        print('{} Total time: {} ({:.4f} s / it)'.format(
            header, total_time_str, total_time / lenn))
...
def save_super_image(x, output):
    root = "/home/mt0/22CS60R54/ssl-sifar-dgx/superimages"
    temp_img = x[0]
...
def create_super_image(x, isLabeled=True):

    if not isLabeled:
        #pick 4 frame
        seg4_image = [x[:,i:i+3, :, : ] for i in range(0, x.shape[1], 6)]
        seg4_image = torch.cat(seg4_image, dim=1)
    

    # padding
    x_padding = torch.zeros((x.shape[0], 3) + x.size()[2:]).cuda()
    x = torch.cat((x, x_padding), dim=1)
    
    #creating super image 3x3
    ...
    if not isLabeled:
        ...
        super_image_2x2 = rearrange(stack, 'b (th tw c) h w -> b c (th h) (tw w)', th=2, c=3)


    
    if not isLabeled:
        return super_image_3x3, super_image_2x2

    return super_image_3x3
