...
class ReversibleSwinTransformerBlock(nn.Module):
    ...
    def F(self, x, mask_matrix):
        ...
        with torch.cuda.amp.autocast(enabled=self.enable_amp):
            ...
            # cyclic shift
            # print("shit size" , self.shift_size, )
            if self.shift_size > 0:
                attn_mask = mask_matrix
                if mask_matrix is None:
                    print("mask matrix is none")
...
class ReversibleSwinTransformer(nn.Module):
    ...
    def __init__(self, duration=8, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, super_img_rows=1, bottleneck=False, fast_backprop=False,
        enable_amp=False,**kwargs
    ):
        super().__init__()
        ...
        self.num_features = int(embed_dim * 2**self.num_layers)         ##embed_dim we are passing as 128
        self.mlp_ratio = mlp_ratio
